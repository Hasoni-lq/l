...
from hsshh import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        checkplugins(path)
        name = "hsshh.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"hsshh/plugins/{shortname}.py")
            name = f"hsshh.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["hsshh.plugins." + shortname] = mod
        LOGS.info(f"[تم تحميل : {shortname}] \n --- hsshh ---")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"hsshh.plugins.{shortname}"
        for i in reversed(range(len(hsshh._event_builders))):
            ev, cb = hsshh._event_builders[i]
            if cb.__module__ == name:
                del hsshh._event_builders[i]
    except BaseException:
        raise ValueError
